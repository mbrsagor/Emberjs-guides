...
class GoogleDriveProvider(FileProvider):
    ...
    def __init__(self, file_id):
        ...
        downloader = MediaIoBaseDownload(self.stream, request)
        done = False
        while done is False:
            status, done = downloader.next_chunk()
            print("Download %d%%." % int(status.progress() * 100))
        self.source = pd.ExcelFile(self.stream)
...
class SheetDataParser(object):
    ...
    def parse(self, sheetName, rawData, headers):
        ...
        self.logger.debug(
            'Found Sheet Title `%s`, hasSubmodule = %s, Feature Column = %d' % (appName, hasSubmodule, featureColumn))
        self.logger.debug('Found Description and Pricing Columns `%s`' % (str(columnMapping)))

        structuredData = self.buildTreeStructure(sheetName, rawData, hasSubmodule, featureColumn, columnMapping)
    ...
    def hasSubmoduleColumn(self, rawData):
        ...
        for row in rawData:
            ...
            if i == 10:
                raise Exception(
                    "Unable to determine if there is Submodule Column. (Cannot find Highest Price or Lowest Price")
    ...
    def processRawDataAdditionalColumns(self, row, columnMapping):
        ...
        values['dependencies'] = [i.strip() for i in values.get('dependencies', '').split(',') if i.strip()];
    ...
    def buildMultipliers(self, row, columnMapping):
        ...
        values['code'], slabConfigStr = values['multiplier'].split(':', 1)
        ...
        try:
            slabConfig = {
                'increment': int(slabConfigStr)
            }
        except Exception:
            try:
                slabConfig = demjson.decode(slabConfigStr)
            except Exception:
                self.logger.error('Unable to parse slabConfig got value (%s)' % slabConfigStr)
                slabConfig = {}

        # print(slabConfig)
        slabConfig['inputType'] = slabConfig.get('inputType', 'text')
        slabConfig['showSlabs'] = slabConfig.get('showSlabs', True)
        slabConfig['slabRange'] = slabConfig.get('slabRange', True)
        slabConfig['increment'] = slabConfig.get('increment', 1);
        slabConfig['customUser'] = slabConfig.get('customUser', False)
        slabConfig['slabTexts'] = slabTexts
        # populate default values for slabconfig
        values['slabConfig'] = slabConfig
        values['slabs'] = slabs
    ...
    def rawDatatoList(self, appName, rawData, hasSubmodule, featureColumn, columnMapping):
        ...
        for row in rawData:
            if isinstance(row[0], str) or not math.isnan(row[0]):
                ...
                moduleGroup = ''.join([x[0] if x[0].isupper() else '' for x in moduleGroupName.split()])
                moduleSelfCode, moduleCode, moduleName, subModule, feature = None, None, None, None, None
            elif isinstance(row[1], str) or not math.isnan(row[1]):
                moduleSelfCode, moduleName = str(row[1]), row[2]
                moduleCode = appName + separator + moduleGroup + separator + moduleSelfCode
                subModule, feature = None, None
            elif hasSubmodule and (isinstance(row[3], str) or not math.isnan(row[3])):
                subModuleName = row[3]
                subModule = ''.join([x[0] if x[0].isupper() else '' for x in subModuleName.split()])
                feature = None
            elif isinstance(row[featureColumn], str) or not math.isnan(row[featureColumn]):
                feature = row[featureColumn]
                featureCode = ''.join([x[0] if x[0].isupper() else '' for x in feature.split()])
                featureCode = (moduleCode if moduleCode else '') + (separator + subModule if subModule else '') + separator + featureCode
            else:
                continue
            ...
            data = dict(moduleGroup=moduleGroup, moduleGroupName=moduleGroupName, moduleCode=moduleCode, moduleSelfCode=moduleSelfCode,
                        moduleName=moduleName, subModule=subModule, subModuleName=subModuleName, feature=feature, featureCode=featureCode, values=additionalValues)
    ...
    def buildTreeStructure(self, appName, rawData, hasSubmodule, featureColumn, columnMapping):
        ...
        for each in listData:
            if each['feature'] is not None:
                ...
                featureData['code'] = each['featureCode']
                featureData.update(each['values'])

                if each['subModule'] is not None:
                    finalData[each['moduleGroup']]['modules'][each['moduleCode']]['submodules'][each['subModule']][
                        'features'][each['feature']] = featureData
                else:
                    finalData[each['moduleGroup']]['modules'][each['moduleCode']]['features'][
                        each['feature']] = featureData
            elif each['subModule'] is not None:

                subModuleData = OrderedDict()
                subModuleData['name'] = each['subModuleName']
                subModuleData['code'] = each['subModule']
                subModuleData['features'] = OrderedDict()
                subModuleData.update(each['values'])

                finalData[each['moduleGroup']]['modules'][each['moduleCode']]['submodules'][
                    each['subModule']] = subModuleData
                ...
                moduleData['selfCode'] = each['moduleSelfCode']
...
class JsonDataExporter(object):
    ...
    def exports(self, data):
        preparedData = self.prepareData(data)
        return self.getContent(preparedData, pretty=False)

    def getContent(self, data, pretty=True):
        if pretty:
...
class PricingDataPrinter(object):
    ...
    def rest(self, values):
        return [k + '=' + str(v) for k, v in values.items() if
                k not in ('name', 'code', 'description', 'ready', 'modules', 'features', 'submodules')]
...
class ExcelDataParser(object):
    ...
    def parseSheet(self, sheetName, sheet):
        ...
        sdp = SheetDataParser()
        appName, structuredData = sdp.parse(sheetName, rawData, sheet.columns)
        return appName, structuredData
    ...
    def fromProvider(self, provider, sheetNameList=[]):
        ...
        for sheetName in sheetNameList:
            ...
            appName, structuredData = self.parseSheet(sheetName, sheet)
